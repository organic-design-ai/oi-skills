...
After installing via `npx skills add`, run the layout installer so child skills land under
`<pkg-dir>/page|widget|ppt|animation/` (not `skills/`):

```bash
<pkg-dir>/scripts/install-to-agent.sh --all -y -g
```
