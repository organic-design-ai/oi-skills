#!/usr/bin/env python3
"""Check runtime dependencies for oi-images-to-gif and suggest install actions."""
...
MIN_PYTHON = (3, 9)
MARKER_PREFIX = "[IMAGES_TO_GIF_ENV]"
...
def run_auto_install(env: dict[str, Any]) -> dict[str, Any]:
    ...
    for command in commands:
        print(f"[oi-images-to-gif] Installing: {command}", file=sys.stderr, flush=True)
...
def _build_agent_prompt(
    ...
    if ready:
        return "Environment ready. Continue oi-images-to-gif workflow."

    lines = ["oi-images-to-gif is missing dependencies:", ""]
    if not python["ok"]:
    ...
    if ai.get("available") and ai.get("commands"):
        ...
        lines.append("```bash")
        lines.append(
            "python3 <pkg-dir>/widget/oi-images-to-gif/scripts/check_env.py --install --json"
        )
        lines.append("```")
