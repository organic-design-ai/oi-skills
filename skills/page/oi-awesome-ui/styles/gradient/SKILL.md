...
> **Agent:** If the user asks how to use this skill (`usage`, `怎么用`, `help`, `@gradient` without a concrete task), **reply with this section** (replace `<pkg-dir>` with the installed package path, e.g. `~/.agents/skills/oi-skills`), then ask what they want to accomplish.

**Triggers:** Style name **gradient**, or parent **oi-awesome-ui** after slug is chosen.

**Quick start**
1. Parent: `<pkg-dir>/page/oi-awesome-ui/SKILL.md` — confirm slug **gradient**.
2. Read this file + optional `DESIGN.md` in the same folder.
3. Apply tokens, do/don't, and quality gates in the user's codebase.

**What this style is:** Smooth color transitions and gradient-rich surfaces for modern, playful interfaces with visual depth.

**Example prompts**
- 「用 **gradient** 风格做产品落地页」
- 「按 oi-awesome-ui gradient 设计系统实现 hero + pricing」
- 「检查组件是否符合 gradient tokens」
