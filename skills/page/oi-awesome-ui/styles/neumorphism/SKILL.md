...
> **Agent:** If the user asks how to use this skill (`usage`, `怎么用`, `help`, `@neumorphism` without a concrete task), **reply with this section** (replace `<pkg-dir>` with the installed package path, e.g. `~/.agents/skills/oi-skills`), then ask what they want to accomplish.

**Triggers:** Style name **neumorphism**, or parent **oi-awesome-ui** after slug is chosen.

**Quick start**
1. Parent: `<pkg-dir>/page/oi-awesome-ui/SKILL.md` — confirm slug **neumorphism**.
2. Read this file + optional `DESIGN.md` in the same folder.
3. Apply tokens, do/don't, and quality gates in the user's codebase.

**What this style is:** Soft, extruded UI elements with inner and outer shadows on monochromatic surfaces for a tactile, embedded look.

**Example prompts**
- 「用 **neumorphism** 风格做产品落地页」
- 「按 oi-awesome-ui neumorphism 设计系统实现 hero + pricing」
- 「检查组件是否符合 neumorphism tokens」
