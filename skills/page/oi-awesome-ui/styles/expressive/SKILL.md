...
> **Agent:** If the user asks how to use this skill (`usage`, `怎么用`, `help`, `@expressive` without a concrete task), **reply with this section** (replace `<pkg-dir>` with the installed package path, e.g. `~/.agents/skills/oi-skills`), then ask what they want to accomplish.

**Triggers:** Style name **expressive**, or parent **oi-awesome-ui** after slug is chosen.

**Quick start**
1. Parent: `<pkg-dir>/page/oi-awesome-ui/SKILL.md` — confirm slug **expressive**.
2. Read this file + optional `DESIGN.md` in the same folder.
3. Apply tokens, do/don't, and quality gates in the user's codebase.

**What this style is:** Vibrant, personality-driven design with bold colors, playful graphics, and dynamic layouts that balance creativity with structure.

**Example prompts**
- 「用 **expressive** 风格做产品落地页」
- 「按 oi-awesome-ui expressive 设计系统实现 hero + pricing」
- 「检查组件是否符合 expressive tokens」
