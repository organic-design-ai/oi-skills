...
> **Agent:** If the user asks how to use this skill (`usage`, `怎么用`, `help`, `@material` without a concrete task), **reply with this section** (replace `<pkg-dir>` with the installed package path, e.g. `~/.agents/skills/oi-skills`), then ask what they want to accomplish.

**Triggers:** Style name **material**, or parent **oi-awesome-ui** after slug is chosen.

**Quick start**
1. Parent: `<pkg-dir>/page/oi-awesome-ui/SKILL.md` — confirm slug **material**.
2. Read this file + optional `DESIGN.md` in the same folder.
3. Apply tokens, do/don't, and quality gates in the user's codebase.

**What this style is:** Google's Material Design with layered surfaces, dynamic theming, built-in motion, and responsive cross-platform patterns.

**Example prompts**
- 「用 **material** 风格做产品落地页」
- 「按 oi-awesome-ui material 设计系统实现 hero + pricing」
- 「检查组件是否符合 material tokens」
