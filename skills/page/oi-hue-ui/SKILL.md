...
name: oi-hue-ui
description: >-
  Design-language generator: create or remix design-system skills from brand signals
  (URL, name, screenshot, local codebase). TRIGGER when user asks for oi-hue-ui,
  hue-ui, or creating/remixing a design-language skill. NOT for fixed visual styles
  (use oi-awesome-ui) or direct page UI without generation.
version: 1.1.0
allowed-tools: [Read, Write, Edit, Glob, Grep, WebFetch, WebSearch]
---
# Oi Hue UI — Design Language Generator
...
**Example prompts**
- 「用 oi-hue-ui 从 cursor.com 生成设计语言 skill」
- 「remix 现有 design skill，色调更暖」
- 「根据截图生成 design-model.yaml + 四套预览」
...
- `styles/examples/ridge/app-screen.html` — SLO overview for `checkout-api`. 8-service sidebar, 3 KPI tiles with sparklines, a 30-day error-budget burn-down chart, 8 log events, and a fake cursor hovering on the selected service. Dev-platform vocabulary (services, alerts, SLO, incidents).
- `styles/examples/stint/app-screen.html` — stint 07 detail view for the `paper` workspace. Sidebar of 7 recent stints with status dots, 3 KPI tiles (completion / days left / at risk), a 14-day burn-down chart with actual vs dashed ideal line, 8-row activity feed, and a fake cursor hovering on the selected stint. Project-tracker vocabulary (stints, tasks, cycles, carryover).
