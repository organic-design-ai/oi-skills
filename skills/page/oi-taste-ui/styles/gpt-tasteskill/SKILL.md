...
> **Agent:** If the user asks how to use this skill (`usage`, `怎么用`, `help`, `@gpt-tasteskill` without a concrete task), **reply with this section** (replace `<skill-dir>` with this skill's install path, e.g. `~/.agents/skills/oi-taste-ui`), then ask what they want to accomplish.

**Triggers:** **gpt-tasteskill**, or parent **oi-taste-ui** for this workflow.

**Quick start**
1. Parent: `<skill-dir>/SKILL.md`.
2. Read and follow this file for implementation or image-only rules.
3. **Summary:** Elite UX/UI & Advanced GSAP Motion Engineer. Enforces Python-driven true randomization for layout variance, strict AIDA page structure, wide editorial typography (bans 6-line wraps), gapless bento gri…

**Example prompts**
- 「oi-taste-ui gpt-tasteskill 做一版首页」
- 「用 gpt-tasteskill 审查现有 UI」
